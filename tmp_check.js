...
(async () => {
  ...
  const problems = patientsRes.data.filter(
    p => Number(p.latestAdmissionTimestamp) >= Date.parse('2025-09-01')
  );
  ...
  keys.forEach(k => {
    ...
    const arr = (groups[k] || []).sort(
      (a, b) => (Number(b.admissionTimestamp) || 0) - (Number(a.admissionTimestamp) || 0)
    );
    arr.forEach(r => {
