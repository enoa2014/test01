...
- [x] **PM-Button 组件优化**（✅ 2025-09-30 完成）
  ...
  - [ ] 添加 ripple 波纹点击效果（可选，低优先级）
  - [ ] 优化 loading 状态的 spinner 样式（低优先级）
  - [ ] 支持 icon-only 模式（低优先级）
...
- [ ] **页面回归测试**（高优先级 - 新增 2025-09-30）
  ...
  - [ ] patient-intake/wizard 页面（已迁移 PM-Input）
    - [ ] 测试完整向导流程（4步）
    - [ ] 验证步骤切换和必填项校验
    - [ ] 测试不同字段类型（text、number）
    - [ ] 验证 hint 提示显示
    - [ ] 测试数据提交功能
    - [ ] 排查已存在患者路径重定向偶发失败问题（当前 E2E 通过 fallback 绕过）
...
- [ ] 更新组件文档与 Component Lab 示例，补充 textarea 属性（`maxlength`、`textareaAutoHeight` 等）
