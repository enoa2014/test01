...
Page({
  ...
  data: {
    ...
    // 从服务端返回的总人数（第一页加载时带上 includeTotal 获取）
    serverTotalCount: 0,
    // 从服务端返回的各状态人数统计
  ...
  async fetchPatients(options = {}) {
    ...
    try {
      ...
      // 仅当请求了 includeTotal 时采用服务端总数，确保“全部”人数稳定
      if (page === 0 && Number.isFinite(result.totalCount)) {
        nextState.serverTotalCount = Number(result.totalCount) || 0;
        if (result.statusCounts && typeof result.statusCounts === 'object') {
          nextState.serverStatusCounts = {
            inCare: Number(result.statusCounts.inCare) || 0,
            pending: Number(result.statusCounts.pending) || 0,
            discharged: Number(result.statusCounts.discharged) || 0,
          };
        }
      }
  ...
  calculateStatsData(patients = []) {
    // 使用服务端返回的总人数（若可用且不小于已加载数量），避免分页导致的“全部”人数波动
    const loaded = patients.length;
    const fromServer = Number(this.data && this.data.serverTotalCount);
  ...
  updateMediaBadges() {
    ...
    const updated = list.map(item => {
      const key = this.resolvePatientKey(item);
      const badges = Array.isArray(item.badges) ? item.badges.filter(b => !(b && b._type === 'media')) : [];
  ...
  async checkUserPermissions() {
    try {
      ...
      // 检查是否有基础权限
      const hasBasicPermission = userManager.hasAnyRole(['social_worker', 'volunteer', 'researcher', 'admin']);
      if (!hasBasicPermission) {
        // 没有基础权限，跳转到权限管理页面
        wx.showModal({
          title: '权限不足',
          content: '您还没有相应的使用权限，请先申请角色或使用邀请码激活权限。',
          showCancel: false,
          success: () => {
            wx.redirectTo({
