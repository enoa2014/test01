...
let ListImage = class ListImage extends common_1.Command {
    get options() {
        return Object.assign(Object.assign({}, (0, common_2.imageCommonOptions)('list')), {
            options: [
                {
                    flags: '-e, --envId <envId>',
                    desc: '环境 Id'
    ...
    execute(envId, options) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            loading.start('数据加载中...')
            const data = yield (0, run_1.listImage)({
                envId,
                limit: Number(limit),
                offset: Number(offset),
            ...
            const tableData = data.map((item) => [
                ...
                item.Size,
                item.ReferVersions.length > 0
                    ? item.ReferVersions.reduce((sum, item) => `${sum}|${item.VersionName}`, '')
                    : '-',
                item.CreateTime,
                item.UpdateTime
            ])
...
ListImage = __decorate([(0, common_1.ICommand)()], ListImage)
