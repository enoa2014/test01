...
async function buildPatientsFromDatabase(options = {}) {
  ...
  // 当请求包含 includeTotal 时，同时统计各 careStatus 数量，供前端稳定显示
  ...
  if (includeTotal) {
    try {
      ...
      statusCounts = {
        ...
        pending: Number(pendingRes && pendingRes.total) || 0,
        discharged: Number(dischargedRes && dischargedRes.total) || 0,
      };
    } catch (error) {
      console.warn('Failed to count status breakdown', error);
    }
  }
