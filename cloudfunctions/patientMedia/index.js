...
function determineCategory({ mimeType, extension }) {
  ...
  if (!ext && mime) {
    const matchedExt = Object.keys(EXTENSION_MIME_MAP).find((key) => EXTENSION_MIME_MAP[key] === mime);
    if (matchedExt) {
      ext = matchedExt;
    }
  }
  ...
  throw makeError("UNSUPPORTED_FILE_TYPE", "不支持的文件类型，请上传 JPG/PNG/WebP/TXT/PDF/Word/Excel 文件");
}
...
async function enrichMediaRecords(records) {
  ...
  records.forEach((record) => {
    const targetThumb = record.thumbFileId || (record.category === "image" ? record.storageFileId : "");
  ...
  return records.map((record) => {
    const formatted = {
      ...
      textPreviewAvailable: record.mimeType === "text/plain" && record.sizeBytes <= TXT_PREVIEW_LIMIT,
      thumbnailUrl: "",
...
async function handleCompleteUpload(event) {
  ...
  try {
    ...
    if ((duplicate && duplicate.data && duplicate.data.length)) {
      await safeDeleteFiles([fileId]);
      throw makeError("MEDIA_DUPLICATE", "重复上传的文件已存在", { existingId: duplicate.data[0]._id });
    }
  } catch (error) {
  ...
  if (category === "image") {
    try {
      ...
      image.cover(THUMBNAIL_WIDTH, THUMBNAIL_HEIGHT, Jimp.HORIZONTAL_ALIGN_CENTER | Jimp.VERTICAL_ALIGN_MIDDLE);
      image.quality(80);
